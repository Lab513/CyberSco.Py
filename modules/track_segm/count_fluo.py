...
@date:      13-11-2019
@brief:     this is a simple code to detect spot
             in a fluorescence image (here, RPF)
...
class COUNT_FLUO():
    ...
    def bytescaling(self, data, cmin=None, cmax=None, high=255, low=0):
        ...
        if low < 0:
            low = 0
        if high < low:
            raise ValueError("`high` should be greater"
    ...
    def detect_cells_fluo(self, addrfluo, debug=[]):
        '''
        '''
        if 0 in debug:
            print('######### In detect_cells_fluo !!!')
            print(f'## addrfluo is {addrfluo} !!!')
        img = cv2.imread(str(addrfluo), 0)      # cv2.IMREAD_UNCHANGED
        if 1 in debug:
            print(f'type(img) is {type(img)} ')
            plt.imshow(img)
        img = self.bytescaling(img)
        img = cv2.GaussianBlur(img, (5, 5), 0)
        img = self.filter2D(img)
        ret2, th2 = cv2.threshold(img, 200, 255, cv2.THRESH_BINARY)
        if 1 in debug:
            plt.imshow(th2)
        self.cntrs_fluo, hierarchy = cv2.findContours(th2.copy(),
                                     cv2.RETR_EXTERNAL,
